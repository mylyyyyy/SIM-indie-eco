...
<body class="antialiased text-slate-600 bg-white" x-data="{ scrolled: false }" @scroll.window="scrolled = (window.pageYOffset > 20)">
    ...
    <span class="font-medium">bukan merupakan milik pribadi masing-masing individu</span>.
            ...
            $roles = [
                ...
                9 => 'ASISTEN MANAGER SUMBER DAYA MANUSIA'
            ];

            $teamContacts = [
                1 => [
                    'nama' => 'Sa diah S.Pd.I.M.Pd.I',
                    'wa' => '08XXX',
                    'email' => 'indiesyafa@gmail.com',
                    'alamat' => '?'
                ],
                2 => [
                    'nama' => 'Puspi S.E',
                    'wa' => '08XXX',
                    'email' => 'direktur@syafagroup.com',
                    'alamat' => '?'
                ],
                3 => [
                    'nama' => 'Bayu Surya S.M',
                    'wa' => '08XXX',
                    'email' => 'mansdm@syafagroup.com',
                    'alamat' => '?'
                ],
                4 => [
                    'nama' => 'Ahmad Hasyim S.E',
                    'wa' => '08XXX',
                    'email' => 'adireksi@gmail.com',
                    'alamat' => '?'
                ],
                5 => [
                    'nama' => 'Yoga Kurniawan W S.Kom',
                    'wa' => '08XXX',
                    'email' => 'it@syafagroup.com',
                    'alamat' => '?'
                ],
                6 => [
                    'nama' => 'Nora Safira A S.H',
                    'wa' => '08XXX',
                    'email' => 'stafflegal@syafagroup.com',
                    'alamat' => '?'
                ],
                7 => [
                    'nama' => 'Muh Roid',
                    'wa' => '08XXX',
                    'email' => 'mkeu@syafagroup.com',
                    'alamat' => '?'
                ],
                8 => [
                    'nama' => 'Resa Desya A S.Pi',
                    'wa' => '08XXX',
                    'email' => 'opr@syafagroup.com',
                    'alamat' => '?'
                ],
                9 => [
                    'nama' => 'Syarif S.H',
                    'wa' => '08XXX',
                    'email' => 'asm@syafagroup.com',
                    'alamat' => '?'
                ],
            ];
        @endphp
                ...
                @for ($i = 1; $i <= 9; $i++)
                    <div class="group bg-white rounded-[2rem] p-4 shadow-lg border border-slate-100 hover:shadow-blue-200/50 hover:border-blue-300 transition-all duration-500 hover:-translate-y-2" data-aos="zoom-in" data-aos-delay="{{ $i * 100 }}">
                        <div class="relative w-full aspect-[4/5] rounded-3xl overflow-hidden bg-slate-100 mb-6 border border-slate-200">
                            ...
                            <img
    src="{{ asset('img/team/' . $i . '.png') }}"
    alt="Team {{ $i }}"
    class="w-full h-full object-cover object-top transition-transform duration-700 group-hover:scale-110"
    onerror="this.onerror=null; this.src='https://ui-avatars.com/api/?name={{ urlencode($teamContacts[$i]['nama'] ?? 'Team '.$i) }}&background=bfdbfe&color=1e3a8a&size=500';">
    ...
    {{ $teamContacts[$i]['nama'] ?? 'Nama Lengkap '.$i }}
</h4>

                            <p class="text-xs font-bold uppercase tracking-wider {{ $i==1 ? 'text-blue-600' : 'text-slate-400' }}">
                                {{ $roles[$i] ?? 'Anggota Tim' }}
                            </p>
                            @if(isset($teamContacts[$i]))
    <div class="mt-4 space-y-2 text-sm text-slate-600">

        {{-- WhatsApp --}}
        <div class="flex items-center justify-center gap-2">
            <i class="fa-brands fa-whatsapp text-emerald-500"></i>
            <a href="https://wa.me/62{{ ltrim($teamContacts[$i]['wa'], '0') }}"
               target="_blank"
        ...
        <div class="flex items-center justify-center gap-2">
            ...
            <a href="mailto:{{ $teamContacts[$i]['email'] }}"
               class="hover:text-blue-600 transition">
                {{ $teamContacts[$i]['email'] }}
            </a>
        </div>
