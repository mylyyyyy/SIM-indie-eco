    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-1">
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 sticky top-24">
                ...
                <form action="{{ route('eco.lhkp.store') }}" method="POST" class="space-y-4">
                    @csrf
                    ...
                    <table class="w-full text-left text-sm text-slate-600">
                        ...
                        <tbody class="divide-y divide-slate-100 bg-white">
                            ...
                            <tr class="hover:bg-slate-50 transition-colors group">
                                ...
                                <td class="px-6 py-4 align-top text-center">
                                    <div class="flex items-center justify-center gap-2 opacity-60 group-hover:opacity-100 transition-opacity">
                                        ...
                                        <form id="delete-form-{{ $lhkp->id }}" action="{{ route('eco.lhkp.destroy', $lhkp->id) }}" method="POST" class="hidden">
                                            @csrf @method('DELETE')
                                        </form>
                                    </div>
                                </td>
                            ...
                            <x-modal name="edit-modal-{{ $lhkp->id }}" focusable>
                                <form method="POST" action="{{ route('eco.lhkp.update', $lhkp->id) }}" class="bg-white rounded-2xl overflow-hidden flex flex-col max-h-[90vh]">
                                    @csrf @method('PUT')
