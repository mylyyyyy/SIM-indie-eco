<x-admin-layout>
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4 animate__animated animate__fadeInDown">
        <div>
            <h2 class="text-3xl font-black text-emerald-800 tracking-tight">Portofolio Eco</h2>
            <p class="text-slate-500 font-medium">Dokumentasi lahan, panen, dan proyek hijau.</p>
        </div>
        
        <div class="flex gap-3 w-full md:w-auto">
            <div class="relative w-full md:w-64 group">
                <input type="text" id="searchInput" placeholder="Cari portofolio..." class="w-full pl-10 pr-4 py-2.5 rounded-xl border border-slate-200 focus:border-emerald-500 text-sm shadow-sm transition-all group-hover:border-emerald-300">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left text-sm text-slate-600" id="portfolioTable">
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-emerald-50/30 transition-colors group">
                        ...
                        <td class="px-6 py-4 text-center">
                            <div class="flex justify-center items-center gap-2">
                                ...
                                <a href="{{ route('eco.portfolios.edit', $item->id) }}" 
                                ...
                                <form action="{{ route('eco.portfolios.destroy', $item->id) }}" method="POST">
                                    @csrf @method('DELETE')
                    ...
                    @empty
                    <tr><td colspan="5" class="px-6 py-12 text-center text-slate-400">Belum ada portofolio eco.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
    ...
    {{-- MODAL TAMBAH (Hanya untuk tambah, Edit sudah pindah halaman) --}}
    <x-modal name="add-portfolio-modal" focusable>
        <form method="POST" action="{{ route('eco.portfolios.store') }}" enctype="multipart/form-data" class="bg-white rounded-2xl flex flex-col max-h-[90vh]">
            @csrf
            <div class="bg-gradient-to-r from-emerald-600 to-teal-700 px-6 py-4 flex justify-between items-center rounded-t-2xl shrink-0">
                <h3 class="text-white font-bold text-lg"><i class="fas fa-leaf text-emerald-200 mr-2"></i> Tambah Proyek Eco</h3>
