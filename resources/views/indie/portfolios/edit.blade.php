<x-admin-layout>
    <div class="max-w-4xl mx-auto">
        <div class="flex items-center gap-4 mb-8">
            <a href="{{ route('indie.portfolios.index') }}" class="w-10 h-10 flex items-center justify-center bg-white border border-slate-200 rounded-xl text-slate-500 hover:bg-emerald-50 hover:text-emerald-600 transition-all shadow-sm">
                <i class="fas fa-arrow-left"></i>
            </a>
            <div>
                <h2 class="text-2xl font-black text-slate-800">Edit Kegiatan</h2>
                <p class="text-sm text-slate-500">Perbarui data distribusi / penjualan.</p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            ...
            <form method="POST" action="{{ route('indie.portfolios.update', $portfolio->id) }}" enctype="multipart/form-data" class="p-8">
                @csrf @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                    ...
                    <div>
                        ...
                        <input type="date" name="completion_date" value="{{ $portfolio->completion_date ? \Carbon\Carbon::parse($portfolio->completion_date)->format('Y-m-d') : '' }}" class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-emerald-500 focus:ring-emerald-500 text-sm" required>
                    </div>

                    <div>
                        <label class="block text-xs font-bold text-slate-500 uppercase mb-2">Tujuan Distribusi (Lokasi)</label>
                    ...
                    <div class="md:col-span-2">
                        <label class="block text-xs font-bold text-slate-500 uppercase mb-2">Keterangan (Volume, dll)</label>
                ...
                <div class="flex justify-end gap-3 pt-6 border-t border-slate-100">
                    <a href="{{ route('indie.portfolios.index') }}" class="px-6 py-3 bg-white border border-slate-200 text-slate-600 rounded-xl text-sm font-bold hover:bg-slate-50 transition-colors">Batal</a>
                    <button type="submit" class="px-6 py-3 bg-emerald-600 text-white rounded-xl text-sm font-bold hover:bg-emerald-700 shadow-lg shadow-emerald-500/30 transition-all transform hover:-translate-y-0.5"><i class="fas fa-save mr-2"></i> Simpan Perubahan</button>
                </div>
            </form>
        </div>
    </div>
