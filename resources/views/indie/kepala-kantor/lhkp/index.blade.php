<x-admin-layout>
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
            <h2 class="text-3xl font-black text-slate-800 tracking-tight">Evaluasi Kinerja SDM</h2>
            <p class="text-slate-500 font-medium">Laporan Evaluasi Pegawai Kantor (LHKP).</p>
        </div>
        
        <div class="flex gap-3">
            <a href="{{ route('indie.kepala-kantor.lhkp.export-pdf') }}" class="bg-white border border-red-200 text-red-600 hover:bg-red-50 px-5 py-2.5 rounded-xl font-bold flex items-center gap-2 shadow-sm transition-all transform hover:-translate-y-0.5">
            ...
            <table class="w-full text-left text-sm text-slate-600">
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-indigo-50/50 transition-colors">
                        ...
                        <td class="px-6 py-4 text-center">
                            <div class="flex justify-center gap-2">
                                ...
                                <form action="{{ route('indie.kepala-kantor.lhkp.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Hapus evaluasi ini?')">
                                    @csrf @method('DELETE')
                    ...
                    <x-modal name="edit-lhkp-modal-{{ $item->id }}" focusable>
                        <form action="{{ route('indie.kepala-kantor.lhkp.update', $item->id) }}" method="POST" class="bg-white rounded-2xl flex flex-col max-h-[90vh]">
                            @csrf @method('PUT')
                            <div class="bg-gradient-to-r from-indigo-600 to-blue-700 px-6 py-4 flex justify-between items-center rounded-t-2xl shrink-0">
                                <h3 class="text-white font-bold text-lg">Edit Evaluasi</h3>
                    ...
                    @empty
                    <tr><td colspan="4" class="px-6 py-12 text-center text-slate-400 font-medium"><i class="fas fa-folder-open text-4xl mb-3 block"></i> Belum ada data evaluasi.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
    ...
    <x-modal name="add-lhkp-modal" focusable>
        <form action="{{ route('indie.kepala-kantor.lhkp.store') }}" method="POST" class="bg-white rounded-2xl flex flex-col max-h-[90vh]">
            @csrf
            <div class="bg-gradient-to-r from-indigo-600 to-blue-700 px-6 py-4 flex justify-between items-center rounded-t-2xl shrink-0">
                <h3 class="text-white font-bold text-lg"><i class="fas fa-plus-circle mr-2"></i> Form Evaluasi Pegawai</h3>
                <button type="button" x-on:click="$dispatch('close')" class="text-indigo-200 hover:text-white transition-colors"><i class="fas fa-times text-xl"></i></button>
            </div>
            <div class="p-6 overflow-y-auto custom-scrollbar space-y-5">
                <div class="grid grid-cols-2 gap-4">
