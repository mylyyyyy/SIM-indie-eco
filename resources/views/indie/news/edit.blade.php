<x-admin-layout>
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4 animate__animated animate__fadeInDown">
        ...
        <a href="{{ route('indie.news.index') }}" class="text-slate-500 hover:text-blue-600 font-bold text-sm flex items-center gap-2">
    ...
    <div class="max-w-4xl mx-auto">
        ...
        <form action="{{ route('indie.news.update', $berita->id_berita) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden animate__animated animate__fadeInUp">
            @csrf
            @method('PUT')
            ...
            <div class="bg-slate-50 px-8 py-5 border-t border-slate-100 flex justify-end gap-3">
                <a href="{{ route('indie.news.index') }}" class="px-6 py-3 bg-white border border-slate-200 text-slate-600 rounded-xl font-bold text-sm hover:bg-slate-50">Batal</a>
