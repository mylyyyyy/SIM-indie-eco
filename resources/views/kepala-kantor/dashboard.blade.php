                    ...
                    <tbody class="divide-y divide-slate-100">
                        ...
                        <tr class="hover:bg-amber-50/50 transition-colors">
                            ...
                            <td class="px-6 py-4 align-top">
                               @php 
    // Cek apakah data sudah berupa array dari Laravel, jika belum baru di-decode
    $kegiatans = is_array($item->rincian_kegiatan) 
                 ? $item->rincian_kegiatan 
                 : (json_decode($item->rincian_kegiatan, true) ?? []); 
@endphp
                                ...
                                @if($item->fotos && $item->fotos->count() > 0)
                                    <div class="flex flex-wrap gap-2">
                                        @foreach($item->fotos as $foto)
                                            {{-- Langsung cetak path_foto karena isinya sudah Base64 (data:image/...) --}}
                                            <a href="{{ $foto->path_foto }}" target="_blank" class="block group relative">
                                                <img src="{{ $foto->path_foto }}" class="w-12 h-12 object-cover rounded-lg border border-slate-200 shadow-sm group-hover:scale-110 transition-transform">
                                            </a>
                                        @endforeach
                                    </div>
